...
//Code Here
let evenNumbers  = mixedNumbers.filter((val) => val % 2 === 0)// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let totalPopulation = populations.reduce((acc, val) => acc + val)//  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(val => val.CP > 200)// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map(val => (val.price * (1 + val.tax)))// Code here
